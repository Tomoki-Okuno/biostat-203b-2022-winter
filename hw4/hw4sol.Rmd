...
# fit the model
model.rf <- randomForest(thirty_day_mort ~ ., data = training_set, ntree = 500)
# predict 30-day mortality using the model
predict_rf <- predict(model.rf, test_set)
# make confusion matrix
...
```
Neural network approach also provided roughly 90% accuracy, although I tried various sequential models.

**Conclusion:**  
In general, random forests and neural networks can have higher prediction accuracy than the others. In this dataset, however, the fact that only 10% of patients died within 30 days may have made little difference in accuracy. Also, executing the correct multiple imputation strategy or spliting data into three to use validation dataset may make the accuracy better.