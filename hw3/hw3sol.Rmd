...
chartevents_tble <- chartevents_tble %>%
  ...
  ungroup() %>%
  select(c(stay_id, itemid, valuenum)) %>%
  # spread valuenum by itemid 
  pivot_wider(names_from = itemid, values_from = valuenum, 
              names_prefix = "chart") %>%
  show_query() %>%
...
**Solution:** My app (app.R) was saved in the `mimiciv_shiny` folder. I made two types of summaries: using 30-day mortality as an indicator (like HW2) and not.
