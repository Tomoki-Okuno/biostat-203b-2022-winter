...
Why are these data files distributed as `.csv.gz` files instead of `.csv` (comma separated values) files? Read the page <https://mimic.mit.edu/docs/iv/> to understand what's in each folder.
    ...
    **`.csv.gz` files are a compressed form of `.csv` files. Compressing a large file can reduce the file size, allowing us to store more files on a hard drive or a web server and transmit them more speedy over the internet than `.csv` files.**
...
**Done.**
    ...
    **`wget` is a command that not only downloads source code and binaries, but also allows you to retrieve an entire website or a specific hierarchy at once. `-nc` option indicates that a file is not retrieved again if the file has already been downloaded.**
    ...
    **Using `-o` option, it is possible to count the words containing each name instead of just counting the lines.**
    ```{bash}
    # wget -nc http://www.gutenberg.org/cache/epub/42671/pg42671.txt
...
**Done.**
    ...
    ```
Explain the output.  
**The output is the print from the 16th line to the 20th line in `pg42671.txt`**. 
