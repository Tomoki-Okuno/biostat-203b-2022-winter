...
3. Briefly describe what bash commands `zcat`, `zless`, `zmore`, and `zgrep` do.
    ...
    **Solution:** What these four commands have in common is to display a compressed file without decompressing it.  
...
**Solution:** I offer two answers as shown below.
    ```{bash} 
    zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | \
      awk -F "," '{print $1}' | sort | uniq | tail -n +2 | wc -l
    zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | \
...
**Solution:**
    ...
    ## 512  history | tail
    ```
    - `history | tail` displays the last ten commands in order from oldest to newest.