...
4. What's the output of following bash script?
    ...
    ```
**By the way, this following code gives almost the same output.**
    ```{bash eval=T}
    ls -l /mnt/mimiciv/1.0/core/ | grep '.gz'
    ```
**This output of the script is a list of all the compressed (`.gz`) files in a long format.**
...
5. Display the first few lines of `admissions.csv.gz`.
    ```{bash} 
    zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | head -3
    ```
