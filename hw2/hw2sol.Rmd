...
```
**Solution:** Next, I used `str` to confirm data types for the three function. Obviously, there are differences between them. For instance, both `read.csv` and `fread` read `subject_id` as `integer` while `read_csv` read this as `double (numeric)`. Also, both `read_csv` and `fread` imported the time as `POSIXct`, but `read.csv` imported as `charactor`.
```{r}
str(tmp_r)
str(tmp_t)
str(tmp_f)
...
**Solution:** I used `read_csv` to import this file as a tibble and sort for `subject_id` and `hadm_id`.
```{r}
...
**Solution:** I tabulated `icustays_tblethe` by `subject_id` and then obtained 53,150 unique `subject_id`'s, which is less than the total number of rows (76,540). Hence, `subject_id` can have multiple ICU stays.
```{r}
...
**Solution:** I used `arrange` to sort `intime` in ascending order, and then I removed the duplicates for `subject_id`. This method should be faster than using `filter` and `min`. As a result, the new table has 53,150 rows.
```{r}
...
**Solution:** I chose bar plots as follows to show the distributions.
- admission year
```{r}
...
```
**Solution:** From the above graph, 0 AM and 7 AM appear to be unusual since the frequency is higher than other times despite a late night and an early morning.
...
# p3 <- ggplot(data = patients_tble) + 
#   geom_histogram(mapping = aes(x = anchor_age, fill = gender)) +
#   labs(title = "") +
#   labs(x = "Anchor age (years)", fill = "Gender")
# max(patients_tble$anchor_age)
...
**Solution:** I imported this and added `label` in `d_labitems.csv.gz` as a new variable. The subset has 16,698,462 rows.
```{r}
labevents_master <- 
...
**Solution:** The following code made `labevents_tble` 52,544 rows. I used `select` to keep only `subject_id` and ten lab measurements, or 11 columns.
```{r}
...
**Solution:** For numeric, I used `summary` to output summary statistics, implying that some appear to have extreme values. Taking this into account, for graphics, I removed values below the 2.5th and above the 97.5th percentile and then displayed these distributions in the following box plots. The distributions for creatinine, glucose, and white blood cells are far from to normal, unlike the others.
```{r}
# numerical summary
...
**Solution:** I read this and added `label` in `d_items.csv.gz` as a new column and obtained 23,679,058 rows. The subset retained all the rows of the original.
```{r}
chartevents_master <-
...
**Solution:** I ran almost the same as Q5-3 and obtained the 53,150 × 6 table, which consists of `subject_id` and five vital measurements.
```{r}
...
if (file.exists("chartevents_tble.rds")) {
  ...
  chartevents_tble <- chartevents_tble %>%
    ...
    left_join(select(icustays_tble, subject_id, intime), 
              by = c("subject_id")) %>%
    # only keep vital measurements after intime
    filter(charttime >= intime) %>%
    # sort charttime in ascending order by subject_id and item_id
    group_by(subject_id, itemid) %>%
    arrange(charttime, .by_group = TRUE) %>%
    # only keep the first vital measurement by group
    slice_head(n = 1) %>%
    # spread label and valuenum
    ungroup() %>%
...
**Solution:** Here is the code to create `mimic_icu_cohort`, which has a 53,065 × 43 table. I set `thirty_day_mort` as `TRUE` if a person died within 30 days and `FALSE` otherwise. I thought first that patients who haven't died were in a different category, though.
```{r}
...
mimic_icu_cohort <- icustays_tble %>%
  ...
  filter(age_hadm > 18) %>%
  # create thirty_day_mort that has T if a patient died <= 30 days and F if not
  mutate(thirty_day_mort = 
           ifelse(is.na(deathtime), "FALSE", 
                  ifelse(as.Date(deathtime) - as.Date(admittime) <= 30, "TRUE",
                         "FALSE"))) %>%
  print(width = Inf) %>%
...
**Solution:** I used bar plots for categorical variables (ethnicity, language, insurance, marital_status and gender) and a box plot for numerical variable (age at hospital admission). 
- `thirty_day_mort` vs ethnicity  
**Brief comment:** The percentage of white patients who died within 30 days was lower than that of those who did not. The percentage of the unknown had the opposite relationship. 
```{r}
...
- For marital status (excluding `NA`), the adult patients who died within 30 days have a smaller proportion of singles than those who did not die, but a larger proportion of widows.
- For gender, there is no significant difference in the ratio of men to women between them.
```{r}
# numerical information
...
```
**Brief comment:** For Bicarbonate and Creatinine, there appear to be large differences between the two categories because one of the medians is closer to the edge of the other box. Also, it may be worth testing whether there are statistically significant differences for glucose and white blood cells as well.
...
```
**Brief comment:** The values for Heart Rate and Respiratory Rate tend to be relatively higher for patients who died within 30 days than those who did not. For the rest, no major differences can be seen.
...
```
**Brief comment:** The proportion of the Cardiac Vascular Intensive Care Unit for patients who died within 30 days is 14 points smaller than that of those who did not. On the other hand, for Medical Intensive Care Unit, the proportion of patients who died within 30 days is 9 points higher. Other than that, there appears to be no significant difference.