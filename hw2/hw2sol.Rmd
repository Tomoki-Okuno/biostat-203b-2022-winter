...
```{r}
    system.time(tmp_baser <- read.csv("/mnt/mimiciv/1.0/core/admissions.csv.gz"))
    system.time(tmp_tidy <- read_csv("/mnt/mimiciv/1.0/core/admissions.csv.gz"))
    system.time(tmp_fread <- fread("/mnt/mimiciv/1.0/core/admissions.csv.gz"))
```
...
**Answer:** I used `fread` to import this file as a tibble.
```{r}
    as_tibble(icustays_tble <- fread("/mnt/mimiciv/1.0/icu/icustays.csv.gz"))
```
...
**Answer:** I made a new variable `min_intime`, minimum of `intime` by `subject_id` and used `filter` to keep only rows where `intime` equals `min_intime` as follows. As a result, the new table has 53,150 rows.
```{r}
    first_icustays_tble <- icustays_tble %>%
      group_by(subject_id) %>%
      arrange(subject_id) %>%
      mutate(min_intime = min(intime)) %>%
      filter(intime == min_intime)
    first_icustays_tble
```
...
**Answer:** I used `fread` to import this file as a tibble.
    ```{r}
    as_tibble(admissions_tble <- read_csv("/mnt/mimiciv/1.0/core/admissions.csv.gz"))
    ```
