...
Next, I used `str` to confirm data types for them. Apparently, there is difference in the (default) parsed data types. For instance, `read.csv` and `fread` read `subject_id` as integer while `read_csv` read this as numeric (double). Also, `read_csv` and `fread` imported the date and time as `POSIXct`, but `read.csv` imported as `factor`.
```{r}
str(tmp_r)
str(tmp_t)
str(tmp_f)
...
ggplot(data = admissions_tble) + 
  ...
  labs(title = "Histogram of admission time by year") +
...
ggplot(data = admissions_tble) + 
  ...
  labs(title = "Histogram of admission time by month") +
...
ggplot(data = admissions_tble) + 
  ...
  labs(title = "Histogram of admission time by month and day") +
...
ggplot(data = admissions_tble) + 
  ...
  labs(title = "Histogram of admission time by week day") +
...
ggplot(data = admissions_tble) + 
  ...
  labs(title = "Histogram of admission time by hour") +
...
ggplot(data = patients_tble, mapping = aes(x = gender, y = anchor_age)) + 
  ...
  geom_boxplot() +
  labs(title = "Boxplot for patient’s age by gender") +
  labs(x = "Gender", y = "Patient’s age")

ggplot(data = patients_tble) + 
  geom_bar(mapping = aes(x = anchor_age, fill = gender)) +
  labs(title = "Histogram for patient’s age by gender") +
...
choice <- c("50912", "50971", "50983", "50902", "50882", 
...
labevents_tble <- labevents_tble %>%
  ...
  distinct(subject_id, itemid, .keep_all = TRUE) #528933 rows
labevents_tble
summary(labevents_tble$valuenum)
```

4. Summarize the lab measurements by appropriate numerics and graphics. memo: label shikkari
```{r}
labevents_tble
ggplot(data = labevents_tble) + 
  geom_bar(mapping = aes(x = factor(itemid), na.rm = TRUE))
```
...
```{r}
chartevents_tble <- 
  read_csv("/mnt/mimiciv/1.0/icu/chartevents_filtered_itemid.csv.gz", lazy = T)
#23679058
...
ggplot(data = chartevents_tble) + 
  geom_bar(mapping = aes(x = factor(itemid)))
```
...
mimic_icu_cohort <- icustays_tble %>%
  ...
  mutate(day_mort = as.Date(deathtime) - as.Date(admittime)) %>%
  mutate(thirty_day_mort = if_else(is.na(day_mort) == 1, 0, if_else(day_mort <= 30, 1, 0)))

mimic_icu_cohort2 <- mimic_icu_cohort %>%
  filter(day_mort>28, day_mort<32) %>%
  select(deathtime, day_mort, thirty_day_mort)
mimic_icu_cohort2
```
