...
d_labitems_tble <- read_csv("/mnt/mimiciv/1.0/hosp/d_labitems.csv.gz")
...
```{r}
file.remove("labevents_tble.rds")
...
ggplot(data = labevents_tble) + 
  ...
  labs(title = "Bar plot for lab item") +
  labs(x = "Item")
ggplot(data = labevents_tble) + 
  geom_bar(mapping = aes(x = factor(label))) +
  labs(title = "Bar plot for lab item") +
  labs(x = "Item")
```
...
d_items_tble <- read_csv("/mnt/mimiciv/1.0/icu/d_items.csv.gz")
...
mimic_icu_cohort <- icustays_tble %>%
  ...
  # create thirty_day_mort that has 1 if a patient died within 30 days and 0 if not
  mutate(thirty_day_mort = 
           ifelse(is.na(deathtime), Inf, 
                  ifelse(as.Date(deathtime) - as.Date(admittime) <= 30, 1,
                         0))) %>%
  print(width = Inf) %>%
